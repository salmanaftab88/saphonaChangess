...
class Maahru extends Component {
  ...
  componentWillReceiveProps = (nextProps, prev) => {
    this.setState({
      todos: nextProps.auraPrintProducts,
    });
  };
  componentDidMount() {
    fetch(Url + "/showSubPrintPro", {
      method: "GET",
    })
      .then(function (res) {
        return res.json();
  ...
  filterList = (e) => {
    ...
    const products = this.props.auraPrintProducts;
          ...
          {filetoshow.map((item) => {
            ...
            return (
              <div className="FCD_product_div" style={{ position: "relative" }}>
                ...
                <Link to={`/print/productDetails/${item.description}`}>
                  <div
...
let NewVM = connect(function (store) {
  return {
    auraPrintProducts: store.imgReducer.auraPrintProducts,
