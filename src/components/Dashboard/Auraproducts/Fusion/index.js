...
class Maahru extends Component {
  ...
  componentWillReceiveProps = (nextProps, prev) => {
    this.setState({
      todos: nextProps.auraFusionProducts,
    });
  };
  componentDidMount() {
    fetch(Url + "/showSubCatFusionPro", {
      method: "GET",
    })
      .then(function (res) {
        return res.json();
  ...
  filterList = (e) => {
    ...
    const products = this.props.auraFusionProducts;
  ...
  render() {
    ...
    console.log("item", this.props.auraFusionProducts);
          ...
          {filetoshow.map((item) => {
            ...
            return (
              <div className="FCD_product_div" style={{ position: "relative" }}>
                ...
                <Link to={`/fusion/productDetails/${item.description}`}>
                  <div
...
let NewVM = connect(function (store) {
  return {
    auraFusionProducts: store.imgReducer.auraFusionProducts,
